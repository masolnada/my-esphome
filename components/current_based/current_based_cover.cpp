// Local fork of esphome 2025.11.4 components/current_based.
//
// Patch: fully-open/fully-closed position commands always drive the motor,
// even when the time-based estimate says the cover is already there. The
// stock component silently drops any command within 1% of the estimated
// position, so a cover stopped while the estimate was clamped at an endpoint
// (estimate runs ahead of real travel) became permanently unresponsive in
// that direction. Endpoint travel is terminated by current sensing anyway,
// so re-driving a truly-at-the-end cover only pulses the relay for ~1s.
...
void CurrentBasedCover::control(const CoverCall &call) {
  ...
  if (call.get_position().has_value()) {
    ...
    if (pos != COVER_OPEN && pos != COVER_CLOSED && fabsf(this->position - pos) < 0.01) {
      // already at target (intermediate targets only — endpoint commands
      // must always run so a stale estimate can't strand the cover)
    } else {
      CoverOperation op;
      if (pos == COVER_OPEN) {
        ...
        op = pos < this->position ? COVER_OPERATION_CLOSING : COVER_OPERATION_OPENING;
      }
